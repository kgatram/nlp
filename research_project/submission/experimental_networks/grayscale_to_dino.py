...
def scale_the_input():
    for i in range(rnd_max):
        x = Image.open(data_dir+'input/'+input_files[i]).convert('L')
        x = transform(x)
...
for epoch in range(epochs):
    ...
    for batch in range(batch_size):
        ...
        rgb_image = Image.open(data_dir+'input/'+input_files[i]).convert('L')
...
with open(f'/cs/home/akhkr1/Documents/{folder_name}/syn_model.txt', 'a') as model_file:
    model_file.write(f'Grayscale input to dino. Linear, softplus, 3 layers, 1024_2_2, with Scaled scale invariant exponential loss. Input 257*384, output 224*224, lr=1e-4, batch size is {batch_size} for {epochs} epochs')
